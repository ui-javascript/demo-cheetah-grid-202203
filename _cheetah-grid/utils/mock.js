const generatePerson = (function () {
  ...
  return function (index) {
    ...
    let birthday = new Date(
      Date.now() - 20 * msOfYear - Math.floor(Math.random() * 15 * msOfYear),
    );
    birthday = new Date(
    ...
    return {
      ...
      fname,
      lname,
      email: `${fname.replace('-', '_')}_${lname.replace(
        '-',
        '_',
...
export function generatePersons(num, initId = 1) {
  const records = [];
  for (let i = 0; i < num; i++) {
...
export function generatePersonsDataSource(num) {
  ...
  return new cheetahGrid.data.CachedDataSource({
    get(index) {
      return array[index]
        ? array[index]
        : (array[index] = generatePerson(index));
    },
    length: num,
  });
}
